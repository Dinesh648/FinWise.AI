...
##optional utility class for color formatting in terminal output
class Colors:
    RESET = "\033[0m"
    BOLD = "\033[1m"
    UNDERLINE = "\033[4m"
...
async def process_agent_response(event):
    """Process and display the agent's response events(for each event).
    For each even in the response it prints the debug information."""
    
    print(f"Event ID: {event.id}, Author: {event.author}")

    has_specified_part = False
...
async def call_agent_async(runner, user_id, session_id, query):
    ...
    """Call the agent asynchronously with the provided user query."""
    ...
    #display the state before processing
    display_state(
    ...
    #Display the state after processing the message
    display_state(
        runner.session_service,
        runner.app_name,
        user_id,
